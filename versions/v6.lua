...
local function walkPath(humanoid, path)
    ...
    for i, waypoint in ipairs(waypoints) do
        ...
        local timeout = 0
        local stuckTime = 0
        local lastPos = humanoid.Parent.HumanoidRootPart.Position
        
        repeat
            wait(0.1)
            timeout = timeout + 0.1
            
            local currentPos = humanoid.Parent.HumanoidRootPart.Position
        ...
        until (humanoid.Parent.HumanoidRootPart.Position - waypoint.Position).Magnitude < 4 or timeout > 5
...
local function WalkToPlayer(playerName)
    ...
    local targetPlayer = Plrs:FindFirstChild(playerName)
    local targetChar = targetPlayer and getCharacter(targetPlayer)
    local targetRoot = targetChar and targetChar:FindFirstChild("HumanoidRootPart")
    
    if not localRoot or not targetRoot or not localHumanoid then return end
    ...
    if plotsFolder then
        for _, p in ipairs(plotsFolder:GetChildren()) do
            if p:GetAttribute("Owner") == playerName then
                plot = p
                break
            end
        end
    ...
    if plot then
        ...
        if bridge then
            if bridge:IsA("Model") then
                bridgePos = bridge:GetPivot().Position
            elseif bridge:IsA("BasePart") then
                bridgePos = bridge.Position
            end
        end
    end
    
    if bridgePos then
        local successToBridge = computeAndWalk(localHumanoid, localRoot.Position, bridgePos)
        if successToBridge then
            computeAndWalk(localHumanoid, localRoot.Position, endPos)
        end
    else
        computeAndWalk(localHumanoid, startPos, endPos)
    end
end
