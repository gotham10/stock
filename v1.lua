...
local function walkPath(humanoid, path)
    ...
    for _, waypoint in ipairs(waypoints) do
        ...
        repeat
            wait(0.1)
            timeout = timeout + 0.1
        until (humanoid.Parent.HumanoidRootPart.Position - waypoint.Position).Magnitude < 4 or timeout > 3
...
local function WalkToPlayer(playerName)
    ...
    if plot then
        ...
        if bridge then
            local bridgePart = bridge:GetPrimaryPart() or bridge:FindFirstChildOfClass("BasePart")
            if bridgePart then
                bridgePos = bridgePart.Position
            end
        end
    end
    
    if bridgePos then
        local success1 = computeAndWalk(localHumanoid, startPos, bridgePos)
        if success1 then
            computeAndWalk(localHumanoid, bridgePos, endPos)
        else
